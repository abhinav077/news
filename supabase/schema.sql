-- Newws initial persistence schema. Apply this file through a reviewed migration.
...
create table if not exists public.sources (
  ...
  constraint sources_listing_url_is_http check (listing_url ~ '^https?://')
);
...
create table if not exists public.articles (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint articles_original_url_is_http check (original_url ~ '^https?://'),
  constraint articles_canonical_url_is_http check (canonical_url is null or canonical_url ~ '^https?://'),
  constraint articles_image_url_is_http check (image_url ~ '^https?://'),
...
create table if not exists public.oxylabs_schedules (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint oxylabs_schedules_schedule_id_digits check (schedule_id ~ '^[0-9]+$')
);
...
create table if not exists public.oxylabs_schedule_runs (
  ...
  status text not null check (status in ('pending', 'running', 'done', 'faulted', 'processed', 'failed')),
  result_status text,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint oxylabs_schedule_runs_provider_run_id_digits check (provider_run_id ~ '^[0-9]+$'),
  constraint oxylabs_schedule_runs_provider_job_id_digits check (provider_job_id is null or provider_job_id ~ '^[0-9]+$'),
  constraint oxylabs_schedule_runs_schedule_provider_run_unique unique (schedule_id, provider_run_id)
);
...
revoke all on table public.sources, public.articles, public.article_analyses,
  public.logs, public.oxylabs_schedules, public.oxylabs_schedule_runs from anon, authenticated;
